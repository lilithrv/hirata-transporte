...
<<<<<<< Updated upstream
    ...
    public VistaInventarioPiezas() {
        construirInterfaz();
        EstiloHirata.aplicarVentana(this, "Mantenimiento de Piezas - Inventario", 1260, 750);
...
>>>>>>> Stashed changes
    private void cargarTabla(boolean soloStockBajo) {
        ...
        for (Pieza p : piezas) {
            ...
            modeloTabla.addRow(new Object[]{
                p.getIdPieza(), p.getTipoPieza().getNombre(), p.getMarca(), p.getModelo(),
                p.getDescripcion(), p.getStockActual(), p.getStockMinimo(),
    ...
    private void cargarSeleccion() {
        ...
        Pieza p = piezas.stream().filter(x -> x.getIdPieza() == id).findFirst().orElse(null);
        if (p == null) return;
    ...
    private Pieza leerFormulario() {
        ...
        Pieza p = new Pieza((TipoPieza) cmbTipo.getSelectedItem(), txtMarca.getText().trim(), txtModelo.getText().trim(), txtDescripcion.getText().trim(), stockActual, stockMinimo);
        if (idSeleccionado != null) p.setIdPieza(idSeleccionado);
        return p;
    }
    ...
    private void eliminar() {
        ...
        int r = JOptionPane.showConfirmDialog(this, "¿Eliminar la pieza seleccionada?", "Confirmar", JOptionPane.YES_NO_OPTION);
        if (r == JOptionPane.YES_OPTION) {
            if (piezaDAO.eliminar(idSeleccionado)) {
                ...
                JOptionPane.showMessageDialog(this, "No se pudo eliminar. Puede estar asociada a un mantenimiento o equipo.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
